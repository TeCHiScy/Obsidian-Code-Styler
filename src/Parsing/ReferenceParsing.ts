...
export function parseReferenceParameters(source: string): ReferenceParameters {
	...
	const filePath =
		passedParameters?.filePath ??
		passedParameters?.file ??
		passedParameters?.path ??
		passedParameters?.link;
	if (typeof filePath === "undefined") throw Error("No file specified");
	const referenceParameters: ReferenceParameters = {
		filePath: filePath,
		language:
			passedParameters?.language ??
			passedParameters?.lang ??
...
export function getLineLimits(
	...
	try {
		...
		let endIndex: number;
		if (referenceParameters.start === null) startIndex = 0;
		else if (typeof referenceParameters.start === "number")
			startIndex = referenceParameters.start - 1;
		else if (
			(referenceParameters.start as string)?.startsWith("/") &&
			(referenceParameters.start as string)?.endsWith("/")
		) {
			const startRegex = new RegExp(
				(referenceParameters.start as string).replace(
					/^\/(.*)\/$/,
					"$1"
				)
			);
			...
			startIndex = lines.findIndex(
				(line) => line.indexOf(referenceParameters.start as string) > -1
			);
		if (referenceParameters.end === null) endIndex = lines.length - 1;
			...
			(referenceParameters.end as string)?.endsWith("/")
		) {
			const endRegex = new RegExp(
				(referenceParameters.end as string).replace(/^\/(.*)\/$/, "$1")
			);
			endIndex = lines.findIndex((line) => endRegex.test(line));
		} else if ((referenceParameters.end as string)?.startsWith("+"))
			endIndex =
				startIndex +
				Number((referenceParameters.end as string).slice(1));
		else
			endIndex = lines.findIndex(
				(line) => line.indexOf(referenceParameters.end as string) > -1
			);
		if (startIndex > endIndex)
			throw Error("Specified Start line is afterthe specified End line");
		else if (startIndex === -1)
			throw Error("Start line could not be found");
		else if (endIndex === -1) throw Error("End line could not be found");
		return {
...
function getLineIdentifier(
	...
	if (typeof lineIdentifier === "undefined") return null;
	else if (typeof lineIdentifier === "number") return lineIdentifier;
