...
export async function getRef(
	...
	try {
		...
		const file = plugin.app.vault.getFileByPath(params.storePath);
		if (!file) {
			throw Error(`Local file '${params.storePath}' not exist`);
		}

		const content = (await plugin.app.vault.read(file)).trim();
...
export async function cleanExternalReferencedFiles(
	...
	for (const id of Object.keys(cache)) {
		...
		await plugin.delete(extRefContentPath(id));
		await plugin.delete(extRefMetadataPath(id));
	}
...
export async function updateExtRef(
	...
	await plugin.update(extRefMetadataPath(params.id), ref.metadata);
	return ref;
}
...
async function readCache(plugin: CodeStylerPlugin): Promise<Cache> {
	const file = plugin.app.vault.getFileByPath(EXT_REF_CACHE);
	if (!file) {
		return {};
	}
	return JSON.parse(await plugin.app.vault.read(file));
}

async function writeCache(cache: Cache, plugin: CodeStylerPlugin) {
	await plugin.update(EXT_REF_CACHE, cache);
}
...
async function readExtRefMetadata(
	...
	const file = plugin.app.vault.getFileByPath(extRefMetadataPath(id));
	if (!file) {
		return {};
	}
	return JSON.parse(await plugin.app.vault.read(file));
